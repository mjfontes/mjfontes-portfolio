...
jQuery(document).ready(function($) {
	...
	if (splashLetters.length) {
		...
		ScrollTrigger.create({
			...
			onUpdate: (self) => {
				...
				const col = Math.round(255 * (1 - progress));
				const lettersOpacity = progress > 0.85 ? Math.max(0, 1 - (progress - 0.85) / 0.15) : 1;
				gsap.set(splashLetters, {
					...
					visibility: lettersOpacity > 0 ? 'visible' : 'hidden',
	...
	if (reelImgs.length) {
		let reelCurrent = 0;
		setInterval(() => {
			reelImgs[reelCurrent].classList.remove('active');
			reelCurrent = (reelCurrent + 1) % reelImgs.length;
			reelImgs[reelCurrent].classList.add('active');
		}, 1200);
	}
